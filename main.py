...
# @app.on_message(pyrogram.filters.command("link") & ~pyrogram.filters.channel)
# def start(client, message):

#     if message.from_user.id not in admin_list:
#         message.reply("Please send a wallpaper as a file.")
#         return

#     printlog(f"{app.get_users(message.from_user.id).first_name} [@{app.get_users(message.from_user.id).username}][{message.from_user.id}] sent a link.")

#     if len(message.command) == 3:
#         if "https://store" in message.command[1] and "gofile.io/download" in message.command[1] and is_valid_link(message.command[1]):
#             message.reply("Added to queue. Please wait.")
#             try:
#                 r = requests.get(message.text, stream = True)
#                 with open(os.path.join("downloads", urllib.parse.unquote(os.path.basename(message.text))), "wb") as wallpaper:
#                     for chunk in r.iter_content(chunk_size = 1024):
#                         if chunk:
#                             wallpaper.write(chunk)
#             except Exception:
#                 message.reply("Something went wrong. Please try again.")
#                 return
#         else:
#             message.reply("Please send a valid gofile.io store link.")
#             return
#     else:
#         message.reply("Please use the following format: /link <gofile.io store link> <compressed/uncompressed>")
#         return

#     if message.command[2] == "compressed":
#         try:
#             im = Image.open(os.path.join("downloads", urllib.parse.unquote(os.path.basename(message.text))))
#             im.save(os.path.join("downloads", urllib.parse.unquote(os.path.basename(message.text))), optimize=True, quality=85)
#         except Exception:
#             message.reply("Something went wrong. Please try again.")
#             return
...
def handle_document(client, message):      
    ...
    if message.from_user.id not in admin_list:
        try:
            app.send_photo(chat_id=-1001701702872, photo=(os.path.splitext(file_path)[0] + ".jpeg"), caption=f"Check the wallpaper and repost it to upload\n\n{upload}")
        except Exception:
