...
async def start(client, message):
    ...
    if message.photo:
        await message.reply("Please send the image as a file.")
    else:
...
async def handle_document(client, message):
    ...
    if u_id in recent_users:
        ...
        return await message.reply("Send one wallpaper at a time.")
    else:
        recent_users.append(u_id)
    printlog(f"{u_name} {username} [{u_id}] sent a document.")
    await asyncio.sleep(randint(1, 8))
    response = await message.reply("Added to queue. Please wait.")
...
async def restart(client, message):
    ...
    scheduler.shutdown()
    os.execl(sys.executable, sys.executable, "tg.py")
